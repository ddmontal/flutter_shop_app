...
class Products with ChangeNotifier {
  ...
  Future<void> fetchAndSetProducts() async {
    ...
    try {
      _items = [];
      (json.decode((await http.get(url)).body) as Map<String, dynamic>).forEach((key, value) {
        _items.add(
          Product(
            id: key,
            title: value['title'],
  ...
  Future<void> updateProduct(String id, Product editedProduct) async {
    final url = Uri.https(Config.firebaseDomain, '$apiPath/$id.json');
  ...
  Future<void> deleteProduct(String id) async {
    ...
    if ((await http.delete(url)).statusCode >= 400) {
